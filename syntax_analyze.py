...
def p_program(p):
    ...
    p[0] = Node("program", [p[1], p[3], p[4], p[6]], [p[2], p[5]])
...
def p_id_array_dec(p):
    ...
    p[0] = Node("id_array_dec", p[2], p[1])

def p_id_array_dec2(p):
    ''' id_array_dec2 : id_dec1
                    | open_brace lit_intposi close_brace array_dec1'''
    if len(p) > 1:
        p[0] = Node("id_array_dec2", p[4],[p[1], p[2], p[3]])
    else:
        p[0] = Node("id_array_dec2", p[1])
        
def p_id_dec1(p):
    ''' id_dec1 : comma id id_dec1
                | empty'''
    if len(p) > 1:
        p[0] = Node("id_dec1", p[3], [p[1], p[2]])
    else:
        pass
...
def p_array_dec1(p):
    ''' array_dec1 : comma id open_brace lit_intposi close_brace array_dec1
                | empty'''
    if len(p) > 1:
        p[0] = Node("array_dec1", p[6], [p[1], p[2], p[3], p[4], p[5]])
    else:
        pass
...
def p_struct_element2(p):
    ''' struct_element2 : comma struct_element1 struct_element2
                        | empty'''
    if len(p) > 1:
        p[0] = Node("struct_element2", [p[2], p[3]], p[1])
    else:
        pass
...
def p_declare_statements(p):
    ''' declare_statements : Generate const_var_dec declare_statements
                            | empty'''
    if len(p) > 1:
        p[0] = Node("declare_statements", [p[2], p[3]], p[1])
    else:
        pass
...
def p_const_var_dec(p):
    ...
    if len(p) > 2:
        p[0] = Node("const_var_dec", [p[2], p[3]], p[1])
    else:
        p[0] = Node("const_var_dec", [p[1], p[2]])
                    
def p_id_array_const(p):
    ''' id_array_const : id id_array_const2 '''
    p[0] = Node("id_array_const", p[2], p[1])
...
def p_id_array_const2(p):
    ...
    if len(p) > 2:
        p[0] = Node("id_array_const2", [p[6], p[8]], [p[1], p[2], p[3], p[4], p[5], p[7]])
                        
def p_id_const1(p):
    ''' id_const1 : comma id equal value id_const1 
                | empty'''
    if len(p) > 1:
        p[0] = Node("id_const1", [p[4], p[5]], [p[1], p[2], p[3]])
    else:
        pass
...
def p_array_const1(p):
    ''' array_const1 : comma id open_brace lit_intposi close_brace equal open_brace value1 close_brace array_const1 
                    | empty'''
    if len(p) > 1:
        p[0] = Node("array_const1", [p[8], p[10]], [p[1], p[2], p[3], p[4], p[5], p[6], p[7], p[9]])
    else:
        pass
                    
def p_id_array_var(p):
    ''' id_array_var : id id_array_var2 '''
    p[0] = Node("id_array_var", p[2], p[1])
    
def p_id_array_var2(p):
    ''' id_array_var2 : var_init id_var1
                    | open_brace lit_intposi close_brace array_init array_var1'''
    if len(p) > 2:
        p[0] = Node("id_array_var2", [p[4], p[5]], [p[1], p[2], p[3]])
    else:
        p[0] = Node("id_array_var2", [p[1], p[2]])

def p_var_init(p):
    ''' var_init : equal value id_var1
                | empty'''
    if len(p) > 1:
        p[0] = Node("var_init", [p[2], p[3]], p[1])
    else:
        pass
...
def p_id_var1(p):
    ''' id_var1 : comma id var_init id_var1
                | empty'''
    if len(p) > 1:
        p[0] = Node("id_var1", [p[3], p[4]], [p[1], p[2]])
    else:
        pass
...
def p_array_init(p):
    ''' array_init : equal open_brace value1 close_brace array_var1 
                | empty'''
    if len(p) > 1:
        p[0] = Node("array_init", [p[3], p[5]], [p[1], p[2], p[4]])
    else:
        pass
...
def p_array_var1(p):
    ''' array_var1 : comma id open_brace lit_intposi close_brace array_init array_var1
                | empty'''
    if len(p) > 1:
        p[0] = Node("array_var1", [p[6], p[7]], [p[1], p[2], p[3], p[4], p[5]])
    else:
        pass
...
def p_num_value(p):
    ''' num_value : id_array id_struct
                ...
                | func_call '''
    if len(p) > 1:
        p[0] = Node("num_value", [p[1], p[2]])
    else:
        p[0] = Node("num_value", p[1])
                
def p_id_array(p):
    ''' id_array : id array'''
    p[0] = Node("id_array", p[2], p[1])
    
def p_array(p):
    ''' array : open_brace lit_intposi close_brace 
            | empty''' 
    if len(p) > 1:
        p[0] = Node("array", None, [p[1], p[2], p[3]])
    else:
        pass

def p_id_array1(p):
    ''' id_array1 : id_array id_array2'''
    p[0] = Node("id_array1", [p[1], p[2]])
    
def p_id_array2(p):
    ''' id_array2 : comma id_array1
                | empty'''
    if len(p) > 1:
        p[0] = Node("id_array2", p[2], p[1])
    else:
        pass            
...
def p_arithmetic_expression_group(p):
    ...
    p[0] = Node("arithmetic_expression", p[2], [p[1], p[3]])
...
def p_arithmetic_operators(p):
    '''arithmetic_operators : plus
                            ...
                            | modulo
                            | times_times'''
    p[0] = Node("arithmetic_operators", p[1])

def p_id_struct(p):
    '''id_struct : period id_array
                | empty'''
    if len(p) > 1:
        p[0] = Node("id_struct", p[2], p[1])
    else:
        pass
...
def p_value1(p):
    '''value1 : value value2'''
    p[0] = Node("value1", [p[1], p[2]])

def p_value2(p):
    '''value2 : comma value1 
            | empty'''
    if len(p) > 1:
        p[0] = Node("value2", p[2], p[1])
    else:
        pass
...
def p_assignment_statements2_more(p):
    ...
    p[0] = Node("assignment_statements2", [p[4], p[7]], [p[1], p[2], p[3], p[5], p[6], p[8]])

def p_assignment_exp(p):
    '''assignment_exp : assignment_operators num_value'''
    p[0] = Node("assignment_exp", [p[1], p[2]])
    
def p_assignment_exp_more(p):
    '''assignment_exp : equal assign_value'''
    p[0] = Node("assignment_exp", p[2], p[1])

def p_assign_value(p):
    '''assign_value : num_value
                    | more_id value'''
    if len(p) > 1:
        p[0] = Node("assign_value", [p[1], p[2]])
    else:
        p[0] = Node("assign_value", p[1])

def p_more_id(p):
    '''more_id : id_array id_struct equal more_id
            | empty'''
    if len(p) > 1:
        p[0] = Node("more_id", [p[1], p[2], p[4]], p[4])
    else:
        pass
...
def p_assignment_operators(p):
    '''assignment_operators : plus_equal
                            ...
                            | modulo_equal
                            | times_times_equal'''
    p[0] = Node("assignment_operators", None, p[1])

def p_more_array(p):
    '''more_array : equal id open_brace lit_intposi close_brace more_array
                | empty'''
    if len(p) > 1:
        p[0] = Node("more_array", p[6], [p[1], p[2], p[3], p[4], p[5]])
    else:
        pass
...
def p_looping_statements(p):
    '''looping_statements : for_statements
                        | while_statements'''
    p[0] = Node("looping_statements", [p[1], p[2]])
...
def p_for_statements(p):
    ...
    p[0] = Node("for_statements", p[9], [p[1], p[2], p[3], p[4], p[5], p[6], p[7], p[8], p[10]])
...
def p_while_statements(p):
    ...
    p[0] = Node("while_statements", [p[3], p[7]], [p[1], p[2], p[4], p[5], p[6], p[8]])
...
def p_inside_statements(p):
    '''inside_statements : statements inside_statements
                        ...
                        | empty'''
    if len(p) > 1:
        p[0] = Node("inside_statements", [p[1], p[2]])
    else:
        pass
...
def p_control_statements(p):
    '''control_statements : Break
                        | Continue
                        | Avoid'''
    p[0] = Node("control_statements", None, p[1])
...
def p_condition(p):
    ...
    if len(p) > 1:
        p[0] = Node("condition", p[3], [p[1], p[2], p[4]])
    else:
...
def p_condition_not(p):
    ''' condition_not   : relational_expression
                        | logical_expression'''
    p[0] = Node("condition_not", p[1])

def p_condition_not_more(p):
    ''' condition_not   : lit_bool'''
    p[0] = Node("condition_not", None, p[1])
...
def p_elif_statement(p):
    ...
    p[0] = Node("elif_statement", [p[3], p[7], p[9]], [p[1], p[2], p[4], p[5], p[6], p[8]])
    
    
def p_else_statement(p):
    ''' else_statement  : Else colon open_bracket inside_statements close_bracket'''
    p[0] = Node("else_statement", p[4], [p[1], p[2], p[3], p[5]])
...
def p_switch_statements(p):
    ...
    p[0] = Node("switch_statements", [p[5], p[8]], [p[1], p[2], p[3], p[4], p[6], p[7], p[9], p[10]])
    
def p_execute(p):
    ''' execute : Execute switch_lit colon statements Break execute1'''
    p[0] = Node("execute", [p[2], p[4], p[6]], [p[1], p[3], p[5]])
    
def p_switch_lit(p):
    ''' switch_lit  : lit_str
                    | lit_intposi
                    | lit_intnega'''
    p[0] = Node("switch_lit", None, p[1])

def p_execute1(p):
    ''' execute1    : execute'''
    p[0] = Node("execute1", p[1])
...
def p_functions(p):
    ''' functions   : Sys id open_par parameters close_par open_bracket declare_statements function_body close_bracket functions
                    | empty'''
    if len(p) > 1:
        p[0] = Node("functions", [p[4], p[7], p[8], p[10]], [p[1], p[2], p[3], p[5], p[6], p[9]])
    else:
        pass
...
def p_parameters(p):
    ''' parameters  : data_type id parameters
                    | empty'''
    if len(p) > 1:
        p[0] = Node("parameters", [p[1], p[3]], p[2])
    else:
        pass
...
def p_function_body(p):
    ''' function_body   : statements function_body
                        ...
                        | empty'''
    if len(p) > 1:
        p[0] = Node("function_body", [p[1], p[2]])
    else:
        pass
                        
def p_return_statement(p):
    ''' return_statement    : Return value'''
    p[0] = Node("return_statement", p[2], p[1])
